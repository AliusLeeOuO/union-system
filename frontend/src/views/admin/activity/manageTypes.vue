<template>
  ...
  >
    <a-form :model="newFeeStandardForm" :rules="newFeeStandardFormRules">
      <a-form-item field="standardName" label="类型名称">
        <a-input v-model="newFeeStandardForm.standardName" :disabled="newTypeAddLoading" />
      </a-form-item>
    </a-form>
  </a-modal>
</template>
...
const { getActivityTypes, newActivityType, deleteActivityType } = useAdminApi()
...
const tableData = reactive<activityType[]>([])
const fetchTypeDataLoading = ref(false)

async function fetchTypes() {
  try {
    fetchTypeDataLoading.value = true
    const { data } = await handleXhrResponse(() => getActivityTypes(), Message)
...
async function addNewTypeFormHandleBeforeOk(done: (closed: boolean) => void) {
  try {
    ...
    await handleXhrResponse(() => newActivityType(newFeeStandardForm.standardName), Message)
...
const newFeeStandardFormRules: Record<string, FieldRule | FieldRule[]> = {
  standardName: [
...
async function fetchDeleteType(id: number) {
  try {
    await handleXhrResponse(() => deleteActivityType(id), Message)
