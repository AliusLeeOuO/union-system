<template>
  ...
  <a-form :model="form" :style="{width:'400px'}" @submit="handleSubmit">
    ...
    <a-form-item field="captchaVal" label="验证码" validate-trigger="input" required>
      <a-input v-model="form.captchaVal" />
    </a-form-item>
    <a-form-item>
      <a-image :src="captchaPicture" show-loader :preview="false" :height="50" :width="120" @click="changeCaptcha" class="captcha"/>
    </a-form-item>
...
const handleSubmit = async (form: { values: Record<string, any>; errors: Record<string, ValidatedError> | undefined }) => {
  if (!form.errors) {
    try {
